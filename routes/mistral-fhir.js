...
// fhirService.js et mistralClient.js

// Les templates de prompts ont été déplacés vers mistralClient.js

/**
 * @swagger
 * /api/mistral-fhir/status:
 *   get:
...
/**
 * @swagger
 * /api/mistral-fhir/{serverId}/natural-query:
 *   post:
...
/**
 * @swagger
 * /api/mistral-fhir/{serverId}/summarize-patient:
 *   post:
...
router.post('/:serverId/summarize-patient', authCombined, async (req, res) => {
  try {
    ...
    // Vérification de la disponibilité de l'API Mistral
    if (!mistralClient) {
      return res.status(503).json({
        success: false,
        message: 'API Mistral non configurée. Veuillez définir la variable d\'environnement MISTRAL_API_KEY'
      });
    }
    ...
    try {
      ...
      const summaryResponse = await mistralClient.chat({
        model: model,
        messages: [
...
/**
 * @swagger
 * /api/mistral-fhir/{serverId}/analyze-observation:
 *   post:
...
`;
      ...
      const analysisResponse = await mistralClient.chat({
        model: model,
        messages: [
...
/**
 * @swagger
 * /api/mistral-fhir/{serverId}/analyze-trends:
 *   post:
...
`;
      ...
      // Appel à l'API Mistral pour l'analyse
      const trendsResponse = await mistralClient.chat({
        model: model,
        messages: [
      ...
      const analysis = trendsResponse.choices[0].message.content.trim();
