...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function searchPatients() {
                    ...
                    try {
                        ...
                        if (!hasResults && !isSecondAttempt) {
                            ...
                            if (serverUrl.includes('hapi.fhir.org')) {
                                ...
                                // Utiliser un count de 250 pour les patients comme demandé
                                widerUrl = `/api/fhir-proxy/hapi/Patient?name=${encodeURIComponent(searchValue)}&_sort=family&_count=250`;
                            } else {
        ...
        if (serverUrl.includes('hapi.fhir.org')) {
            // Utiliser le proxy pour contourner les limitations CORS
            // Utiliser un count de 250 pour les patients comme demandé
            initialUrl = `/api/fhir-proxy/hapi/Patient?family=${encodeURIComponent(searchValue)}&_sort=family&_count=250`;
        } else {
            // URL directe pour les serveurs locaux (déjà sur le même domaine)
    ...
    function loadPatientConditions(patientId, serverUrl) {
        ...
        if (serverUrl.includes('hapi.fhir.org')) {
            ...
            const fhirPath = `Condition?patient=${patientId}&_sort=-recorded-date&_count=50`;
    ...
    function loadPatientObservations(patientId, serverUrl) {
        ...
        if (serverUrl.includes('hapi.fhir.org')) {
            // Utiliser le proxy pour contourner les limitations CORS
            // Utiliser un count réduit (50) pour éviter les erreurs 429 (Too Many Requests)
            url = `/api/fhir-proxy/hapi/Observation?patient=${patientId}&_sort=-date&_count=50`;
        } else {
            // URL directe pour les serveurs locaux (déjà sur le même domaine)
    ...
    function loadPatientMedications(patientId, serverUrl) {
        ...
        if (serverUrl.includes('hapi.fhir.org')) {
            // Utiliser le proxy pour contourner les limitations CORS
            // Utiliser un count réduit (50) pour éviter les erreurs 429 (Too Many Requests)
            url = `/api/fhir-proxy/hapi/MedicationRequest?patient=${patientId}&_sort=-date&_count=50`;
        } else {
            // URL directe pour les serveurs locaux (déjà sur le même domaine)
            ...
            .then(data => {
                ...
                if (data.entry && data.entry.length > 0) {
                    ...
                    relatedPersons.forEach(person => {
                        ...
                        // Construire le HTML sans utiliser de template literals complexes
                        let personHtml = '<h4 style="margin-top: 0; color: #333; font-size: 1.1rem; display: flex; align-items: center; gap: 10px;">' +
                        ...
                        if (person.relationship) {
                            personHtml += '<p><strong>Relation:</strong> ' + formatRelationship(person.relationship) + '</p>';
                        }
                        
                        if (person.telecom) {
                            personHtml += '<p><strong>Contact:</strong> ' + formatTelecom(person.telecom) + '</p>';
                        }
                        
                        if (person.address && person.address.length > 0) {
                            personHtml += '<p><strong>Adresse:</strong> ' + formatAddress(person.address[0]) + '</p>';
                        }
    ...
    function loadPatientCoverage(patientId, serverUrl) {
        ...
        if (serverUrl.includes('hapi.fhir.org')) {
            // Utiliser le proxy pour contourner les limitations CORS
            // Utiliser un count réduit (50) pour éviter les erreurs 429 (Too Many Requests)
            url = `/api/fhir-proxy/hapi/Coverage?beneficiary=Patient/${patientId}&_count=50`;
        } else {
            // URL directe pour les serveurs locaux (déjà sur le même domaine)
            url = `${serverUrl}/Coverage?beneficiary=Patient/${patientId}&_count=100`;
        }
